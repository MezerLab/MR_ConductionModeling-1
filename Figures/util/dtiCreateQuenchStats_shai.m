function fg = dtiCreateQuenchStats_shai(fg, statFiberName, statPointName, perPoint, img, fiberSummary, pointCompute,mpRange)
%
...
% 2018: SB added the option of 'nanmedian_med' where the median is
%       calculated over the medial nodes of each streamlines
...
if notDefined('img')
    ...
    switch(lower(fiberSummary))
        ...
        case 'nanmedian'
            param.stat = cellfun(@nanmedian,vals);
        case 'nanmean'
            param.stat = cellfun(@nanmean,vals);
        case 'nanmedian_med'
            for ll=1:length(fg.fibers);
                xCoords = fg.fibers{ll}(1,:);
                Lf = size(fg.fibers{ll},2);
                if Lf<60
                    param.stat(ll) = nan;
                    continue
                end
                xCoords([1:20,(Lf-20):Lf])=nan;
                [~,medialXind] = min(abs(xCoords-0));
                
                if isempty(medialXind)
                    param.stat(ll) = nan;
                    continue;
                elseif length(medialXind)>1
                    medialXind=medialXind(1);
                end
                
                medialPoint = vals{ll}((medialXind-10):(medialXind+10));
                param.stat(ll) = nanmedian(medialPoint);
            end
        otherwise
